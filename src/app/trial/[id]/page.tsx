...
function OutcomeChart({ logs, accent }: { logs: DailyLog[]; accent: string }) {
  const validLogs = logs.filter(l => l.primary_outcome !== null)
  if (validLogs.length < 2) return (
    <div style={{ height: '100px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <p style={{ fontSize: '11px', color: '#ffffff20' }}>Not enough data to chart yet</p>
    </div>
  )

  const W = 280
  const H = 110
  const PAD = { top: 10, right: 8, bottom: 22, left: 22 }
  ...
  const xScale = (i: number) => PAD.left + (i / Math.max(validLogs.length - 1, 1)) * chartW
  const yScale = (v: number) => PAD.top + chartH - ((v - minV) / Math.max(maxV - minV, 1)) * chartH
  ...
  const baselineEndIdx = validLogs.findIndex(l => l.phase === 'intervention')
  const baselineX = baselineEndIdx > 0 ? xScale(baselineEndIdx) : null
    ...
    <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ overflow: 'visible' }}>
      ...
      {baselineX && (
        <>
          ...
          <text x={baselineX - 3} y={PAD.top + 7} textAnchor="end"
            style={{ fontSize: '7px', fill: '#ffffff25', fontFamily: 'DM Mono, monospace' }}>baseline</text>
          <text x={baselineX + 3} y={PAD.top + 7} textAnchor="start"
            style={{ fontSize: '7px', fill: '#ffffff25', fontFamily: 'DM Mono, monospace' }}>intervention</text>
        </>
      )}
      <polygon points={areaPoints} fill={`${accent}12`} />
      ...
      <text x={PAD.left} y={H - 4} textAnchor="start"
        style={{ fontSize: '7px', fill: '#ffffff20', fontFamily: 'DM Mono, monospace' }}>
...
function isTrialComplete(trial: Trial, logs: DailyLog[]): boolean {
  if (!trial.started_at || !trial.protocol?.versions) return false
  const activeVersion = trial.protocol.versions?.find(
    (v: any) => v.version === trial.protocol.active_version
  )
  if (!activeVersion?.phases) return false
  const totalDays = activeVersion.phases.reduce((sum: number, p: any) => sum + p.duration_days, 0)
  const daysSinceStart = Math.floor((Date.now() - new Date(trial.started_at).getTime()) / 86400000)
...
export default function ResultsPage() {
  ...
  async function loadData() {
    ...
    const { data: logData } = await supabase
      .from('daily_logs')
      .select('date, phase, primary_outcome, adherence')
      .eq('trial_id', trialId)
  ...
  async function concludeTrial() {
    ...
    // Regenerate interpretation as final
  ...
  // Trial state logic
  const isConcluded = trial.status === 'concluded'
  const isActive = trial.status === 'active'
  const trialComplete = isActive && isTrialComplete(trial, logs)
  const isPreliminary = isActive && !trialComplete
    ...
    <div style={{ ...s.root, '--accent': colors.accent } as any}>
      ...
      <header style={s.header}>
        <div style={s.headerInner}>
          ...
          {isPreliminary && <span style={s.prelimBadge}>Preliminary</span>}
          {trialComplete && !isConcluded && <span style={s.completeBadge}>Ready to conclude</span>}
          {isConcluded && <span style={s.concludedBadge}>Concluded</span>}
        </div>
        <div style={s.headerInner}>
          <div>
        ...
        {isPreliminary && (
          <div style={s.headerInner}>
            <p style={s.prelimNote}>
              Your trial is still running — {logs.length} days logged so far. These findings will sharpen as you keep going.
            </p>
          </div>
        )}
        {trialComplete && !isConcluded && (
          <div style={s.headerInner}>
            <p style={s.completeNote}>
              You've completed all phases. Ready to get your final verdict?
            </p>
            <button
              onClick={concludeTrial}
              disabled={concluding}
              style={{ ...s.concludeHeaderBtn, background: colors.accent, color: '#0a0a0a' }}
            >
              {concluding ? 'Concluding...' : 'Conclude trial →'}
            </button>
          </div>
        )}
      </header>
        ...
        {generating && (
          <div style={s.generatingCard}>
            <div style={s.generatingInner}>
              <div style={s.spinner} />
              <div>
                <p style={s.generatingTitle}>
                  {isConcluded ? 'Generating final verdict...' : 'Analysing your data...'}
                </p>
        ...
        {interpretation && !generating && (
          <>
            ...
            <div style={s.card}>
              ...
              <div style={s.splitRow}>
                ...
                <div style={s.splitLeft}>
                  ...
                  <div style={s.scoreRow}>
                    <div style={s.scoreBlock}>
                      ...
                      <p style={s.scoreVal}>
                        {interpretation.baseline_avg?.toFixed(1)}
                        <span style={s.scoreDenom}>/10</span>
                      </p>
                    </div>
                    <div style={s.scoreArrow}>
                      <span style={{ color: isPositive ? '#4ade80' : '#f87171', fontSize: '18px' }}>
                        {isPositive ? '↑' : '↓'}
                      </span>
                    ...
                    <div style={s.scoreBlock}>
                      ...
                      <p style={{ ...s.scoreVal, color: isPositive ? '#4ade80' : '#f87171' }}>
                        {interpretation.intervention_avg?.toFixed(1)}
                        <span style={s.scoreDenom}>/10</span>
                      </p>
                    </div>
                  </div>
                ...
                <div style={s.splitRight}>
                  ...
                  <OutcomeChart logs={logs} accent={colors.accent} />
                  <div style={s.chartLegend}>
                    <span style={s.legendItem}>
                      <span style={{ ...s.legendDot, background: colors.accent }} />logged
                    </span>
                    <span style={s.legendItem}>
                      <span style={{ ...s.legendDot, background: '#f87171' }} />missed
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div style={s.card}>
              ...
              {/* Conclude section — state-aware */}
              {isPreliminary && (
                <div style={s.concludeSection}>
                  <p style={s.concludeHint}>
                    Your trial is still running. Keep logging for a more reliable result — or force-conclude early if you're done.
                  </p>
                  <button onClick={concludeTrial} disabled={concluding} style={s.concludeBtn}>
                    {concluding ? 'Concluding...' : 'Force conclude early'}
                  </button>
                </div>
              )}
              ...
              {trialComplete && !isConcluded && (
                <div style={s.concludeSection}>
                  <div>
                    <p style={s.concludeReadyTitle}>Your trial is complete</p>
                    <p style={s.concludeHint}>All phases done. Concluding will lock in your final verdict.</p>
                  </div>
                  <button
                    onClick={concludeTrial}
                    disabled={concluding}
                    style={{ ...s.primaryBtn, background: colors.accent, color: '#0a0a0a' }}
                  >
                    {concluding ? 'Concluding...' : 'Conclude and get final verdict'}
                  </button>
                </div>
              )}
            </div>
...
const s: Record<string, React.CSSProperties> = {
  ...
  headerInner: { maxWidth: '680px', margin: '0 auto', display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px', flexWrap: 'wrap' },
  backBtn: { background: 'none', border: 'none', color: '#ffffff35', fontSize: '12px', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" },
  ...
  completeBadge: { marginLeft: 'auto', fontSize: '11px', color: '#4ade80', background: '#22c55e15', border: '1px solid #22c55e30', borderRadius: '20px', padding: '3px 10px' },
  concludedBadge: { marginLeft: 'auto', fontSize: '11px', color: '#ffffff50', background: '#ffffff08', border: '1px solid #ffffff15', borderRadius: '20px', padding: '3px 10px' },
  ...
  completeNote: { fontSize: '13px', color: '#4ade80cc', lineHeight: 1.5 },
  concludeHeaderBtn: { border: 'none', borderRadius: '8px', padding: '8px 16px', fontSize: '13px', fontWeight: 500, cursor: 'pointer', flexShrink: 0 },
  main: { position: 'relative', zIndex: 10, maxWidth: '680px', margin: '0 auto', padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: '14px' },
  ...
  cardTitle: { fontSize: '11px', fontWeight: 500, color: '#ffffff55', letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: '14px' },
  splitRow: { display: 'flex', gap: '20px', alignItems: 'flex-start', flexWrap: 'wrap' },
  splitLeft: { flex: '1 1 260px', display: 'flex', flexDirection: 'column', gap: '12px' },
  splitRight: { flex: '1 1 220px', display: 'flex', flexDirection: 'column', gap: '6px' },
  chartLabel: { fontSize: '10px', color: '#ffffff28', textTransform: 'uppercase', letterSpacing: '0.06em' },
  chartLegend: { display: 'flex', gap: '12px', marginTop: '4px' },
  legendItem: { display: 'flex', alignItems: 'center', gap: '5px', fontSize: '10px', color: '#ffffff30' },
  legendDot: { width: '5px', height: '5px', borderRadius: '50%', flexShrink: 0 },
  ...
  cachedPill: { fontSize: '11px', color: '#ffffff20', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '20px', padding: '3px 10px' },
  ...
  caveatLabel: { fontSize: '10px', color: '#ffffff28', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '6px' },
  caveatText: { fontSize: '13px', color: '#ffffff60', lineHeight: 1.6 },
  flagList: { display: 'flex', flexDirection: 'column', gap: '10px' },
  flagRow: { display: 'flex', alignItems: 'flex-start', gap: '10px' },
  flagDot: { fontSize: '11px', color: '#fbbf24', flexShrink: 0, marginTop: '1px' },
  ...
  concludeSection: { marginTop: '20px', paddingTop: '16px', borderTop: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '16px', flexWrap: 'wrap' },
  concludeReadyTitle: { fontSize: '13px', fontWeight: 500, color: '#4ade80cc', marginBottom: '4px' },
  concludeHint: { fontSize: '12px', color: '#ffffff30', lineHeight: 1.5 },
  concludeBtn: { background: 'transparent', border: '1px solid rgba(255,255,255,0.12)', borderRadius: '8px', padding: '8px 16px', fontSize: '12px', color: '#ffffff35', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif", flexShrink: 0 },
  regenRow: { display: 'flex', justifyContent: 'center', paddingBottom: '24px' },
  regenBtn: { background: 'transparent', border: 'none', fontSize: '12px', color: '#ffffff18', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" },
}